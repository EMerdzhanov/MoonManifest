            ...
            return SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  _CycleTimeline(
                    lunarState: state,
                    engine: ref.read(moonPhaseEngineProvider),
                  ),
...
class _CycleTimeline extends StatelessWidget {
  ...
  final MoonPhaseEngine engine;

  const _CycleTimeline({required this.lunarState, required this.engine});

  @override
  Widget build(BuildContext context) {
    final now = engine.now;

    // Compute the key dates for the current/surrounding cycle
    final phaseInfo = engine.currentPhase(now);
    final nextNewMoon = phaseInfo.nextNewMoon;
    final nextFullMoon = phaseInfo.nextFullMoon;

    // Work backwards to find the cycle anchors
    // The previous new moon is the start of the current cycle
    // We can derive it: if we're before the full moon, the cycle started at
    // the new moon before us. If after, same thing.
    ...
    if (lunarState.phase == MoonPhase.newMoon ||
      ...
      if (lunarState.phase == MoonPhase.waxing) {
        // phaseStart is the start of waxing; new moon was before that
    ...
    return Column(
      ...
      children: [
        ...
        ...entries.asMap().entries.map((e) {
          ...
          return _buildTimelineRow(context, entry, isLast);
        }),
      ],
    );
  }
  ...
  Widget _buildTimelineRow(
      BuildContext context, _TimelineEntry entry, bool isLast) {
